...
MongoClient.connect(url, (err, client) => {
    ...
    app.listen( //port, () => {
      console.log('listening on available port...connected to terminalsAPI')
    )
})

router.get("/", function(req, res, next) {
    //res.send("API is working properly");
    db.collection('terminaldata').find().toArray(function(err, results) {
        console.log(results)
        // send HTML file populated with tvscollection here
        if (err) return console.log(err)
        res.json({termndata: results})
    })
});
